<template lang="html">
  <section>
    <h2 class="text-cyan font-dosis font-bold text-3xl">Usuarios</h2>

    <div class="admin-box">
      <div class="admin-box-inner">
        <div class="flex -mx-4 md:flex-row flex-col">
    ...
    <div class="admin-box">
      <div class="admin-box-title flex flex-wrap justify-between items-center">
        <h2 class="flex-1 text-teal font-dosis font-bold xs:mb-4 leading-none">Registros de usuario por fecha</h2>
    ...
    <div class="admin-box">
      <div class="admin-box-title flex">
        <h2 class="text-cyan font-dosis font-bold leading-none">Registros de usuario por rango de edades</h2>
      </div>
      <div class="admin-box-inner">
        <base-chart type="bar" :chart-data="agesChart" />
      </div>
    </div>

    <div class="admin-box">
      <div class="admin-box-title flex">
        <h2 class="text-cyan font-dosis font-bold leading-none">Usuarios activos</h2>
      </div>
      <div class="admin-box-inner">
        <base-chart type="bar" :chart-data="usersChart" />
      </div>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    async load(draw = true) {
      ...
      var stats = await this.$http.get('/admin/stats/candidates', {
        params
      });

      this.datesChart = stats.data.dates;
      this.agesChart  = stats.data.ages;
      this.usersChart = stats.data.users;
    },
    setDimension(value) {
